...
import { NbtParseException } from "lib/IntegratedDynamicsClasses/NBTFunctions/NbtParseException";
...
export class INbtPathExpression {
   ...
   */
  match(
    nbts: Array<Tag<IntegratedValue>> | Tag<IntegratedValue>
  ...
  test(nbts: Array<Tag<IntegratedValue>> | Tag<IntegratedValue>): boolean {
    ...
    return (
      this.match([nbts[0] as Tag<IntegratedValue>])
        .getMatches()
        .filter(
   ...
   */
  matchContexts(
    _executionContexts: Array<NbtPathExpressionExecutionContext>
  ): NbtPathExpressionMatches {
    throw new Error(
