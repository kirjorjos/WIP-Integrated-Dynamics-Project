function NBTtoAST(nbt) {
  ...
  if (typeof nbt === "string" && nbt.startsWith("integrateddynamics:")) {
    return { operator: ((typeof nbt === "string") ? extractKeyByInternalName(nbt) : NBTtoAST(nbt)), args: [] };
  }

  // ✅ Unwrap full ValueType wrapper
  if (nbt._type === "integrateddynamics:valuetype" && nbt.typeName === "integrateddynamics:operator") {
  ...
  if ("value" in nbt && "valueType" in nbt && !("serializer" in nbt)) {
    return {
      type: capitalizeFirstChar(nbt.valueType.replace("integrateddynamics:", "")),
      value: nbt.value
    };
  }
          ...
          args: [
            {
              ...
              args: (value.values ?? []).map(val => {
                ...
                return {
                  type: val.valueType?.replace("integrateddynamics:", "") ?? "Unknown",
                  value: val.value
                };
              })
            }
