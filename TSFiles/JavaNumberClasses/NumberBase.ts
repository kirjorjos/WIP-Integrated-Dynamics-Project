import { TypeBit, TypeInt32, TypeInt64, TypeLambda, TypeNumber, TypeNumericString } from '../types.ts';
...
export abstract class NumberBase {
    ...
    protected abstract initializeBits(decimal: TypeNumericString): TypeInt32 | TypeInt64;

    protected static divmod2(decimal: TypeNumericString): [TypeNumericString, number] {
        let quotient = "";
        let carry = 0;
     ...
     */
    private static mul2Add(decimal: TypeNumericString, bit: 0 | 1): TypeNumericString {
    let carry = bit;
    let result = "";
    for (let i = decimal.length - 1; i >= 0; i--) {
        const d = decimal.charCodeAt(i) - 48;
        const prod = d * 2 + carry;
        result = (String.fromCharCode(48 + (prod % 10)) + result) as TypeNumericString;
    ...
    protected static bitsToSignedDecimal(bits: TypeBit[]): TypeNumericString {
        ...
        return sign === 1 && decimal !== "0" ? "-" + decimal as TypeNumericString : decimal;
    }
    ...
    public static add(num1: TypeNumber, num2: TypeNumber): TypeNumber {
        let [higherOrderNum, lowerOrderNum] = (num1.getOrder() < num2.getOrder()) ? [num2, num1] : [num1, num2];
