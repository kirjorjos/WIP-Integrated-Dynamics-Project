 ...
 */
export class NbtPathExpressionParseHandlerListSlice implements INbtPathExpressionParseHandler {
    ...
    handlePrefixOf(nbtPathExpression: string, pos: number): HandleResult {
        ...
        return new HandleResult(new NbtPathExpressionParseHandlerListSlice.Expression(start, end, step),
                3 + startString.length + endString.length + (stepString == null ? 0 : 1 + stepString.length));
    }

    public static newStartEndStepStream(start: number, end: number, step: number): Array<number> {
        end -= start - 1;
                ...
                .flatMap((executionContext) => {
                    ...
                    if (nbt.getType() == Tag.TAG_LIST) {
                        ...
                        return NbtPathExpressionParseHandlerListSlice.newStartEndStepStream(start, actualEnd, step)
                                .map((i) => new NbtPathExpressionExecutionContext(tag.get(i), executionContext));
                    }
                    return null;
                })
                .filter((e) => e != null)
