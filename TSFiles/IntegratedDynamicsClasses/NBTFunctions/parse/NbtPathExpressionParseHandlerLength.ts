 ...
 */
export class NbtPathExpressionParseHandlerLength implements INbtPathExpressionParseHandler {

    public handlePrefixOf(nbtPathExpression: string, pos: number): HandleResult {
        if (nbtPathExpression.substring(pos, Math.min(pos + 7, nbtPathExpression.length)) != ".length") {
        ...
        return new NbtPathExpressionMatches(executionContexts
                .map(executionContext => {
                    ...
                    if (nbt.getType() == Tag.TAG_LIST) {
                        ...
                        return new NbtPathExpressionExecutionContext(IntTag.valueOf(new Integer(tag.size())), executionContext);
                    } else if (nbt.getType() == Tag.TAG_COMPOUND) {
                        let tag = nbt as CompoundTag;
                        return new NbtPathExpressionExecutionContext(IntTag.valueOf(new Integer(tag.getAllKeys().length)), executionContext);
                    }
                    return null;
                })
                .filter((e) => e != null)
