...
export class CompoundTag extends Tag<IntegratedValue> {
	...
	setAll(keys: string[], values: Tag<IntegratedValue>[]): CompoundTag {
		if (keys.length != values.length) throw new Error(`Keys (length ${keys.length}) is not the same as values (${values.length})`)
		let data = { ...this.data };
	...
	static fromJSON(data: string): CompoundTag {
		const jsonStr = data
			...
			.replace(/(:\s*)([A-Za-z0-9_]*[A-Za-z][A-Za-z0-9_]*)(?=\s*[,}])/g, '$1"$2"')	//quote many values
			.replace(
			...
			for (const key of Object.keys(obj)) {
				switch(typeof obj[key]) {
					...
					case "string":
						...
						if (str.match(/\d*[Bb]/)) obj[key] = new ByteTag(new Integer(parseInt(str.slice(0, -1))));
						if (str.match(/\d*[Ss]/)) obj[key] = new IntTag(new Integer(parseInt(str.slice(0, -1))));
						else if (str.match(/\d*[Ll]/)) obj[key] = new LongTag(new Long(parseInt(str.slice(0, -1))));
						else if (str.match(/\d*[FfDd]/)) obj[key] = new DoubleTag(new Double(parseInt(str.slice(0, -1))));
						else if (str.match(/d{1,}/)) obj[key] = new IntTag(new Integer(parseInt(obj[key])));
						break;
					default:
	...
	compoundSubset(subset: CompoundTag): boolean {
		for (const key of subset.getAllKeys()) {
			...
			if (subValue instanceof CompoundTag && superValue instanceof CompoundTag) {
				...
				if (subValueArr.every((v, i) => superValueArr[i]?.equals(v))) return true;
				return false;
			}
		}
		return true;
